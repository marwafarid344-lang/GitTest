...
export default function SurveyArPage() {
  ...
  useEffect(() => {
    setIsMounted(true)
    try {
      const savedStep = localStorage.getItem("chameleon-step-ar")
      const savedAnswers = localStorage.getItem("chameleon-answers-ar")
      const savedOther = localStorage.getItem("chameleon-other-ar")
  ...
  useEffect(() => {
    ...
    if (step > 0 && step <= TOTAL) {
      localStorage.setItem("chameleon-step-ar", step.toString())
      localStorage.setItem("chameleon-answers-ar", JSON.stringify(answers))
      localStorage.setItem("chameleon-other-ar", otherText)
    } else if (step > TOTAL) {
      localStorage.removeItem("chameleon-step-ar")
      localStorage.removeItem("chameleon-answers-ar")
      localStorage.removeItem("chameleon-other-ar")
    }
        ...
        <AnimatePresence mode="wait" initial={false}>
          ...
          {step === 0 && (
            <motion.div key="intro" {...FADE_UP} transition={DUR}>
              ...
              <motion.div 
                className="flex items-center gap-2 mb-4 mr-1"
          ...
          {q?.id === "text-a-read" && showChallenge ? (
            <motion.div key="challenge-splash" {...FADE_UP} transition={DUR} className="relative text-center flex flex-col items-center justify-center min-h-[50vh] rounded-3xl p-10">
               ...
               <h2 className="text-4xl md:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-rose-500 mb-6 drop-shadow-lg leading-tight relative z-10">
                 التحدي هيبدأ دلوقتي!
               </h2>
               <p className="text-xl md:text-2xl text-white/80 mb-10 max-w-lg font-light leading-relaxed relative z-10">
                 تقدر تفرق بين كتابة الإنسان والذكاء الاصطناعي؟<br/>
                 <span className="font-semibold text-white">ورينا قوة ملاحظتك.</span>
               </p>
